...
def main(args):
    ...
    if config.task == "dti_dg":
        ...
        datamodule = TDCDataModulecocopy(
            task_dir,
            drug_featurizer,
            target_featurizer,
            device=device,
        ...
        config.watch_metric = "val/aupr"
        config.latent_activation = "ReLU"
        datamodule = DTIDataModulecocopy(
            task_dir,
            drug_featurizer,
            target_featurizer,
            device=device,
    ...
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        opt, T_0=config.lr_t0,T_mult=config.lr_t_mult,eta_min=config.eta_min
    )
    ...
    for epo in range(config.epochs):
        ...
        for i, batch in tqdm(
            ...
            # # 输出大于1的pred值
            # greater_than_one = pred[pred > 1.0]

            # # 只在有大于1的值时输出
            # if greater_than_one.numel() > 0:  # 检查是否有元素
            #     print("Pred values greater than 1:", greater_than_one)

            # 对 pred 应用上限限制
       
            # pred = torch.clamp(pred, max=1.0)#避免精度导致的溢值
        ...
        #     logg.info(
        #         f"Training at Knowledge Distillation Epoch {epo + 1} with avg loss {s_loss:8f}"
        #     )
        #     logg.info(
        ...
        if epo % config.every_n_val == 0:
            with torch.set_grad_enabled(False):
                ...
                if val_results[config.watch_metric] > max_metric:
                    ...
                    #     f"{save_dir}/{config.run_id}{config.target_featurizer}_the10_best_model_epoch{epo:02}.pt"
                    # )
                    # torch.save(
    ...
    try:
        with torch.set_grad_enabled(False):
            ...
            model_save_path = Path(f"{save_dir}/{config.run_id}{config.target_featurizer}{config.task}_resnet_9_30_best_model.pt")
            torch.save(
                model_max.state_dict(),
                model_save_path,
            )
